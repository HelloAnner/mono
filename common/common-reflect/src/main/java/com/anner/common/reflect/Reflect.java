package com.anner.common.reflect;
...
public class Reflect {
    ...
    public Method exactMethod(String name, Object... args) throws ReflectException {
    ...
    public Reflect create(Object... args) throws ReflectException {
        ...
        // signature if primitive argument types are converted to their wrappers
            ...
            try {
                ...
                return on(objenesis.newInstance(type()));
            } catch (Exception ex) {
     ...
     * @return 是否匹配构造函数
     */
    public boolean matchConstructor(Object... args) {
        Class<?>[] types = types(args);
        try {
            Constructor<?> constructor = type().getDeclaredConstructor(types);
    ...
    private boolean match(Class<?>[] declaredTypes, Class<?>[] actualTypes) {
        if (declaredTypes.length == actualTypes.length) {
            for (int i = 0; i < actualTypes.length; i++) {
                if (actualTypes[i] == NULL.class || actualTypes[i] == null)
                    continue;
